...
exports.getQuestion = (req, res) => {
  try {
    ...
    console.log(`🎮 [${mode} lvl${level}] ${question} = ${answer}`);
...
exports.saveResult = async (req, res) => {
  try {
    ...
    console.log('💾 Сохраняем результат:', { userId, mode, score, level, correct, total });
...
async function checkAndAwardTrophies(userId, mode, score, level) {
  ...
  for (const trophy of modeTrophies) {
    ...
    if (earned) {
      ...
      console.log(`🏆 Выдан трофей: ${trophy.name} для пользователя ${userId}`);
    }
  }
